...
const TP = (() => {
  ...
  function runEngine(match, h2h, td) {
    ...
    if (r1 && r2) { const rd = r2 - r1; const rp = cl(50 + rd * 0.3, 10, 90); if (M.elo.conf > 0) { M.elo.p1 = M.elo.p1 * 0.7 + rp * 0.3; M.elo.conf = Math.min(M.elo.conf + 20, 100); } else { M.elo.p1 = rp; M.elo.conf = Math.min(Math.abs(rd) * 1.5, 80); } M.elo.det += ` #${r1} vs #${r2}`; }
    const C = calcC(M), R = calcR(M, C), T = calcT(M, C);
    const MK = calcMK(C, match, h2h, sf, bo5, M.odds, td);
    return { M, C, R, T, MK, sf, bo5, dq: dQ(M), r1, r2 };
  }

  // Models (compact — same logic as v6)
  ...
  function mH2H(h, m) { const r = { name: 'H2H', icon: '⚔️', p1: 50, conf: 0, det: '', w1: 0, w2: 0 }; if (!h || !h.H2H || !h.H2H.length) return r; let w1 = 0, w2 = 0; h.H2H.forEach(x => { if (iW(x, m.first_player_key)) w1++; else w2++; }); const tot = w1 + w2; r.p1 = cl((w1 / tot) * 100, 8, 92); r.conf = Math.min(tot * 18, 100); r.det = `${w1}-${w2}`; r.w1 = w1; r.w2 = w2; return r; }
  ...
  function cfat(res, md) { const t = new Date(md || new Date()); let m7 = 0, m3 = 0; res.slice(0, 10).forEach(m => { if (!m.event_date) return; const da = Math.floor((t - new Date(m.event_date)) / 86400000); if (da >= 0 && da <= 7) m7++; if (da >= 0 && da <= 3) m3++; }); const sc = m3 * 2 + m7 * 0.5; return { sc, m: m7, lb: sc >= 5 ? '🔴Stanco' : sc >= 3 ? '🟡Norm' : '🟢Fresco' }; }
  ...
  function calcR(M, C) { const ag = mAg(M); const ac = C.bd.filter(b => b.active).reduce((s, b) => s + (b.conf || 0), 0) / Math.max(C.ac, 1); const sc = +C.conf * 0.35 + ag * 0.35 + ac * 0.30; return { sc: sc.toFixed(1), tier: sc >= 72 ? 'gold' : sc >= 52 ? 'silver' : sc >= 32 ? 'bronze' : 'skip', stars: sc >= 82 ? 5 : sc >= 67 ? 4 : sc >= 52 ? 3 : sc >= 37 ? 2 : 1, ag: ag.toFixed(0), ac: ac.toFixed(0) }; }
